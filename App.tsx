...
import { ClipboardListIcon, CogIcon, ViewGridIcon, SearchIcon, UploadIcon, DocumentDownloadIcon, ChartPieIcon } from './components/icons/Icons';
...
const ReportCard: React.FC<{ title: string; children: React.ReactNode; }> = ({ title, children }) => (
    <div className="bg-white p-6 rounded-lg shadow-md border border-metro-border">
        <h3 className="text-xl font-bold text-metro-text-heading mb-4 border-b pb-2 border-gray-200">{title}</h3>
        {children}
    </div>
);
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-8">
            <div className="text-center">
                <h2 className="text-3xl font-extrabold text-metro-text-heading">Monthly Operations & Performance Report</h2>
            ...
            <div className="flex justify-between items-center bg-gray-50 p-3 rounded-md border">
                <p className="text-sm text-gray-500">Generated on: <span className="font-semibold">{formattedDate} at {formattedTime}</span></p>
                <button
                    disabled
            ...
            <ReportCard title="Executive Summary">
                <p className="text-metro-text-body leading-relaxed">
                    This month, the Kochi Metro system demonstrated strong operational performance and reliability. Fleet availability remained high, exceeding targets, while energy consumption was optimized effectively. The AI Induction Planner played a crucial role in proactively identifying maintenance needs, resulting in a 15% reduction in unscheduled service interruptions compared to the previous month. Key initiatives included the successful completion of the annual brake system audit and the rollout of new branding on five trainsets, meeting all contractual deadlines.
                </p>
            </ReportCard>
            ...
            <div className="grid md:grid-cols-2 gap-8">
                ...
                <ReportCard title="Maintenance Overview">
                    <div className="space-y-2 divide-y divide-gray-100">
                        ...
                        <MetricItem label="Health Score Improvement" value="+4.5 pts (avg)" />
                    </div>
                </ReportCard>
            </div>
            ...
            <ReportCard title="AI Induction Plan Analysis">
                 <p className="text-metro-text-body mb-4">
                    The AI planner consistently produced optimal induction lists, requiring minimal human intervention. Analysis shows a strong correlation between AI recommendations and improved fleet health scores.
                </p>
                <div className="space-y-2 divide-y divide-gray-100">
                ...
                <div className="mt-4 p-3 bg-blue-50 border-l-4 border-metro-blue rounded-r-md">
                    <p className="text-sm text-metro-blue-dark"><span className="font-bold">Key Insight:</span> The AI successfully prioritized three trains with 'ExpiringSoon' fitness certificates, ensuring they were scheduled for inspection well before their expiry dates and preventing any potential service disruptions.</p>
                </div>
            </ReportCard>
...
const LandingPage: React.FC<{ onLaunchPlanner: (tab: PlannerTab) => void; user: User; onLogout: () => void; }> = ({ onLaunchPlanner, user, onLogout }) => {
    
    const stations = [
        "Aluva", "Pulinchanodu", "Companypady", "Ambattukavu", "Muttom", "Kalamassery",
        ...
        "Vadakkekotta", "S. N. Junction", "Thrippunithura"
    ];
    
    const plannerButtons: { title: string; tab: PlannerTab | null; enabled: boolean }[] = [
    ...
    const StatCounter: React.FC<{ finalValue: number; label: string; suffix?: string; }> = ({ finalValue, label, suffix = '' }) => {
        ...
        return (
            <div ref={ref} className="text-center">
                ...
                <p className="text-sm md:text-base font-semibold text-metro-text-body mt-2">{label}</p>
            </div>
        );
    };
  ...
  return (
    <div className="bg-white text-metro-text-body font-sans">
      ...
      <header className="sticky top-0 bg-white/90 backdrop-blur-md z-20 shadow-sm border-b border-metro-border">
        <nav className="container mx-auto px-6 py-3 flex justify-between items-center">
          <a href="#" className="flex items-center space-x-3">
            <svg className="w-8 h-8 text-metro-blue" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" />
            </svg>
            <div>
              <h1 className="text-xl font-bold text-metro-text-heading">Kochi Metro</h1>
              <h2 className="text-sm font-semibold text-metro-blue -mt-1">AI Induction Planner</h2>
            </div>
          </a>
          <div className="hidden md:flex items-center space-x-8 font-semibold">
              <a href="#about" onClick={(e) => handleNavClick(e, 'about')} className="hover:text-metro-blue transition-colors">About</a>
              <a href="#map" onClick={(e) => handleNavClick(e, 'map')} className="hover:text-metro-blue transition-colors">Metro Map</a>
              <a href="#technology" onClick={(e) => handleNavClick(e, 'technology')} className="hover:text-metro-blue transition-colors">Technology</a>
              <a href="#stations" onClick={(e) => handleNavClick(e, 'stations')} className="hover:text-metro-blue transition-colors">Stations</a>
          </div>
      ...
      <section 
        ...
        style={{ backgroundImage: `url('https://images.unsplash.com/photo-1568530656475-573a62a63216?q=80&w=2940&auto=format&fit=crop')` }}
      >
        <div className="absolute inset-0 bg-black/60"></div>
        <div className="relative z-10 text-center p-4 max-w-4xl mx-auto">
          <h1 className="text-4xl md:text-6xl font-extrabold mb-4 leading-tight drop-shadow-lg">India's 8th Inter-modal Metro System</h1>
          <p className="text-lg md:text-xl mb-8 font-light max-w-2xl mx-auto drop-shadow-md">
            Harnessing AI to drive operational excellence and intelligent trainset induction for the nation's most integrated public transport network.
          </p>
          <button
        ...
        <div className="relative z-10 grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 mt-16 px-4 max-w-5xl mx-auto">
            {plannerButtons.map(button => (
                <button
                    ...
                    title={!button.enabled ? 'Coming soon' : `Navigate to ${button.title}`}
      ...
      <section className="bg-white py-16">
        <div className="container mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-8">
            ...
            <StatCounter finalValue={27} label="Operational Route (km)" suffix=".4" />
            <StatCounter finalValue={50} label="Total Fleet Size" />
            <StatCounter finalValue={99} label="AI Plan Accuracy (%)" suffix="%" />
        </div>
      </section>
        ...
        <div className="grid md:grid-cols-2 gap-16 items-center">
            <div>
                ...
                <p className="text-lg text-metro-text-body leading-relaxed mb-4">
                    Kochi Metro is a state-of-the-art rapid transit system serving the city of Kochi. As the first metro in India to be fully integrated with other modes of transport like buses and ferries, it offers a seamless and efficient travel experience for commuters.
                </p>
                 <p className="text-lg text-metro-text-body leading-relaxed mb-4">
                    Beyond its technological prowess, Kochi Metro is celebrated for its commitment to social inclusion and sustainability. It has set a national benchmark by employing women from the Kudumbashree mission and members of the transgender community in its operations.
                </p>
                 <p className="text-lg text-metro-text-body leading-relaxed">
                    Each station is uniquely designed with themes reflecting Kerala's rich heritage, from its Western Ghats biodiversity to its maritime history. The system also champions green energy, with extensive use of solar power to reduce its carbon footprint.
                </p>
            </div>
            <div className="flex justify-center">
                <img src="https://images.unsplash.com/photo-1570125909232-eb263c186922?q=80&w=2940&auto=format&fit=crop" alt="Kochi Metro train at a station" className="rounded-lg shadow-2xl object-cover w-full h-auto max-w-md" />
            </div>
        </div>
      </section>
      ...
      <section id="map" className="py-24 px-6 bg-metro-light-bg">
          <div className="container mx-auto text-center">
              <h2 className="text-4xl font-bold text-metro-text-heading mb-4">Interactive Metro Network Map</h2>
              <p className="text-lg text-metro-text-body max-w-3xl mx-auto mb-10">Explore the live Kochi Metro network, view stations, and plan your journey with this interactive map.</p>
      ...
      <section id="technology" className="container mx-auto py-24 px-6">
        <div className="text-center mb-16 max-w-3xl mx-auto">
            <h2 className="text-4xl font-bold text-metro-text-heading mb-4">Technology Behind the Plan</h2>
            <p className="text-lg text-metro-text-body">A streamlined, three-stage process transforms complex data into actionable intelligence.</p>
        </div>
        <div className="grid md:grid-cols-3 gap-8 items-center max-w-5xl mx-auto relative">
            ...
            <div className="absolute top-1/2 left-0 w-full flex justify-around hidden md:flex">
                <ArrowRightIcon className="w-8 h-8 text-metro-blue bg-white px-1" style={{marginLeft: '25%'}} />
                <ArrowRightIcon className="w-8 h-8 text-metro-blue bg-white px-1" style={{marginRight: '25%'}}/>
            </div>
            <div className="relative flex flex-col items-center text-center p-6 bg-white rounded-lg shadow-lg border border-metro-border">
                <div className="mb-4 flex items-center justify-center w-16 h-16 rounded-full bg-metro-blue/10 text-metro-blue"><UploadIcon className="w-8 h-8"/></div>
                <h3 className="text-xl font-bold text-metro-text-heading mb-2">1. Data Ingestion</h3>
                <p className="text-metro-text-body text-sm">Real-time fleet status, maintenance logs, and operational constraints are ingested via CSV.</p>
            </div>
            <div className="relative flex flex-col items-center text-center p-6 bg-white rounded-lg shadow-lg border border-metro-border">
                <div className="mb-4 flex items-center justify-center w-16 h-16 rounded-full bg-metro-blue/10 text-metro-blue"><CpuChipIcon className="w-8 h-8"/></div>
                <h3 className="text-xl font-bold text-metro-text-heading mb-2">2. AI Analysis</h3>
                <p className="text-metro-text-body text-sm">Google's Gemini model analyzes the data against customizable rules and priority weights.</p>
            </div>
             <div className="relative flex flex-col items-center text-center p-6 bg-white rounded-lg shadow-lg border border-metro-border">
                <div className="mb-4 flex items-center justify-center w-16 h-16 rounded-full bg-metro-blue/10 text-metro-blue"><DocumentDownloadIcon className="w-8 h-8"/></div>
                <h3 className="text-xl font-bold text-metro-text-heading mb-2">3. Optimized Plan</h3>
                <p className="text-metro-text-body text-sm">An optimal, conflict-free induction plan is generated with detailed, explainable reasoning.</p>
            </div>
        </div>
      </section>
      ...
      <section id="stations" className="py-24 px-6 bg-metro-light-bg">
        <div className="container mx-auto max-w-5xl">
          <div className="text-center mb-12">
            <h2 className="text-4xl font-bold text-metro-text-heading mb-4">Operational Stations</h2>
            <p className="text-lg text-metro-text-body">The Blue Line currently serves 25 stations from Aluva to Thrippunithura.</p>
          </div>
          <div className="bg-white rounded-lg shadow-xl border border-metro-border overflow-hidden">
            <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
              {stations.map((station, index) => (
                <li key={station} className="p-4 border-b border-r border-gray-100 flex items-center space-x-3">
                  <span className="flex items-center justify-center w-7 h-7 font-bold text-sm bg-metro-blue text-white rounded-full flex-shrink-0">{index + 1}</span>
                  <span className="font-semibold text-metro-text-heading">{station}</span>
                </li>
              ))}
            </ul>
          </div>
      ...
      <section id="cta" className="bg-metro-blue text-white">
        <div className="container mx-auto py-20 px-6 text-center">
          <h2 className="text-3xl font-bold mb-4">Ready to Revolutionize Your Operations?</h2>
          <p className="text-lg text-blue-100 max-w-2xl mx-auto mb-8">
            Move beyond spreadsheets and manual planning. Embrace the future of metro operations management.
          </p>
          <button
            onClick={() => onLaunchPlanner('dashboard')}
...
const PlannerPage: React.FC<{ 
  ...
  const [highlightedTrainId, setHighlightedTrainId] = useState<number | null>(null);
  const [activeTab, setActiveTab] = useState<PlannerTab>(initialTab);
  
  // State for dashboard filters
  const [searchQuery, setSearchQuery] = useState('');
  const [fitnessFilter, setFitnessFilter] = useState<'All' | FitnessStatus>('All');
  const [jobCardFilter, setJobCardFilter] = useState<'All' | JobCardStatus>('All');
  ...
  const TABS: { id: PlannerTab; label: string; icon: React.FC<React.SVGProps<SVGSVGElement>> }[] = [
  ...
  const handleGeneratePlan = useCallback(async () => {
    if (trains.length === 0) {
        setApiError("No train data available. Please upload a CSV or generate mock data.");
        setActiveTab('plan');
        return;
    }
    setIsLoading(true);
    ...
    try {
      ...
      console.error(err);
      setApiError('Failed to generate induction plan. Please check your API key and network connection, then try again.');
    } finally {
      setIsLoading(false);
    }
  ...
  const handleRegenerateData = useCallback(() => {
    ...
    setRecommendation(null);
    setApiError(null);
    setNotification({ message: `Generated ${newTrains.length} new mock trainsets.`, type: 'success' });
    setHighlightedTrainId(null);
  ...
  const handleFileUpload = useCallback((file: File) => {
    ...
    reader.onload = (e) => {
        ...
        try {
            ...
            setApiError(null);
            setHighlightedTrainId(null);
            setNotification({ message: `Successfully loaded ${newTrains.length} trainsets from ${file.name}.`, type: 'success' });
            setActiveTab('dashboard');
        } catch (err) {
            console.error(err);
  ...
  const handleSaveRules = (newRules: string, newWeights: PriorityWeights) => {
    ...
    setNotification({ message: 'AI decision rules and priorities have been updated.', type: 'success' });
  };

  const handleTrainSelect = useCallback((trainId: number | null) => {
  ...
  const filteredTrains = useMemo(() => {
    return trains.filter(train => {
      ...
      const fitnessMatch = fitnessFilter === 'All' || train.fitnessCertificateStatus === fitnessFilter;
      const jobCardMatch = jobCardFilter === 'All' || train.jobCardStatus === jobCardFilter;
        ...
        <div className="mt-6">
           {activeTab === 'dashboard' && (
            <div className="space-y-6">
                ...
                />
                <TrainDashboard trains={filteredTrains} highlightedTrainId={highlightedTrainId} />
            </div>
           )}
           {activeTab === 'plan' && (
...
const App: React.FC = () => {
    ...
    const [initialPlannerTab, setInitialPlannerTab] = useState<PlannerTab>('dashboard');
